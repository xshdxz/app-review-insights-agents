...
- **混合检索第一次有数字**：修完「两路召回」（D-19）后，用本地模型 `BAAI/bge-small-zh-v1.5`
  （新增可选 extra `[embeddings]`）在 21 条查询上实测——recall@5 **0.595 → 0.714**、MRR 0.873 → 0.885，
  逐条看 **4 条改善 / 0 条变差 / 17 条持平**；代价是 precision@5 0.319 → 0.286（召回换精度）。
  recall@3 的 +0.024 只相当于「一个期望项」，文档里写成「有信号但不显著」而不是「提升了」，并记下
  0.6/0.4 权重仍未调优。混合那一路**不进 CI**（要 2GB 级模型），数字按文档里的命令本地复现
...
- **修 D-18（查询改写的负收益）**：检索评测先证明「无条件多路合并」是负收益（recall@3 0.595 → 0.524、
  MRR 0.873 → 0.746），根因是**跨查询比较归一化分数**（每条查询 top1 都是 1.0）。修法取最保守的一条——
  **只在原查询零结果时才启用改写**，并把策略收敛到唯一入口 `rag/answer.gather_evidence`（UI 与评测脚本
  共用；脚本原先另抄了一份合并逻辑）。回退路的合并改为**按名次轮转**而非比分。复测 `rewrite_triggered = 0`、
  各项指标与纯 FTS 逐位相同；改写的上界仍未量到，如实记在 `docs/retrieval-eval.md`
- **修 D-20（`--live-rewrite` 只活在文档里）**：评测脚本定义了这个开关却从未读过它，配了密钥的机器上
  一次「离线」评测也会真实调用模型（21 次）。改为显式判定，并把「改写」「混合」两个可选配置拆成互相
  独立的开关，未启用时如实写进 `skipped`
