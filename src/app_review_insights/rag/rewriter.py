"""查询改写器：把用户自然语言问题扩展为多条等价检索表达，**用于原查询零结果时的兜底召回**。

无条件多路合并经检索评测实测为负收益（recall@3 0.595 → 0.524、MRR 0.873 → 0.746），
触发条件因此收在 `rag.answer.gather_evidence` 一处：命中即止，零结果才改写（D-18）。
